...
class ACO(object):
    def __init__(self, ant_num=100, maxIter=200, alpha=2, beta=1, gama=1, rho=0.7, Q=2):
            ...
            for i in range(int(self.auto_ant_num(iter_num))): # 种群规模自适应
                ...
                while True:  # 对剩余的点进行访问
                    ...
                    for k in range(len(next_point_list)):
                        ...
                        # print(success_point,':',np.power(self.phero_mat[point_index1][next_point_list[k]],self.alpha),':',np.power(self.auto_eta_mat(point_index1,next_point_list[k],j),self.beta),':',\
                        #     np.power(float(angle/np.pi),self.gama))

                    print('up_proba:',up_proba)
                ...
                #--------------------------------------计算信息素--------------------------------------
                # 保存这一次迭代的最优解
                # self.success_set.append(ant_path)
                # success_point += 1
    ...
    def display(self):
        ...
        if len(self.final_best_path) > 0:            
            ...
            with open('showPoint_success.txt','a+') as fw:
